...
| 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|
| 2.1 | `scripts/fetch_futures.py` | ✅ | AKShare 获取 ZC/J/JM 主力行情，输出 `data/futures.json`（带 fallback） |
| 2.2 | `scripts/fetch_stocks.py` | ✅ | AKShare 获取8只煤炭股，输出 `data/stocks.json`（带 fallback） |
| 2.3 | `.github/workflows/daily_market.yml` | ✅ | 工作日9:30触发，commit并push |
| 2.4 | `index.html` 前端改造 | ✅ | 已实现三级降级：`fetchFuturesFromLocal()` / `fetchStocksFromLocal()` 优先读 `data/*.json`，失败降级东方财富 API，再降级模拟数据 |
| 2.5 | `scripts/fetch_weekly.py` | ✅ | 输出 `data/weekly.json`（表3 外生指标9项）；`REAL_SOURCES` 为防御式真实数据源接入点：布伦特原油已尝试经 AKShare 拉真实价，其余 fallback 占位。已接入 `weekly_data.yml` |
| 2.6 | `index.html` 外生指标页 | ✅ | `loadExogenous()` 优先读 `data/weekly.json` 按 id 覆盖显示值，失败回退 `EXO_DEFAULT` 内置默认；涨绿跌红，备注标注数据来源 |
| 2.7 | `index.html` 外生数据联动分析页 | ✅ | 新增 `getExo()`/`exoMetrics()` 进口成本模型（汇率/热值折算/港杂费 透明可调）。比价表「动力煤/澳煤价差」「煤油比价」与「进口套利信号」、打分表「比价套利」维度均由 `weekly.json` 的澳煤现货价/海运费/布伦特实时计算；`renderSpreadTable()` 随期货/外生数据刷新联动 |
| 2.8 | `index.html` 库存/供需信号数据驱动 | ✅ | 新增 `computeEntrySignals()`（参考阈值 `INV_REF`/`ENV_THRESH`）：从 IndexedDB 最新+上一期录入计算「库存周期信号」(港口库存 vs 均值)、打分表「供给紧张度」(产量+进口 环比)、「需求强度」(电厂日耗 环比)。无录入数据回退静态默认并显示「待录入」。保存/删除/批量导入/启动时经 `refreshSignalsFromEntries()` 联动刷新；综合研判叙述改用真实库存值；并修复综合评分「看多」判定的字符串比较 bug |
| 2.9 | `index.html` 进口成本模型补完税口径 | ✅ | 系数收成可调配置 `IMPORT_MODEL`（汇率/增值税13%/关税/港杂/热值折算）。进口平价改为 `CIF×汇率×(1+关税)×(1+增值税)+港杂 →热值折算`，到岸完税成本由 916→1022 元/吨（补回 13% 增值税）。`exoMetrics()` 返回 `impTax` 税负分解；比价面板副标题透明展示模型假设，价差行显示「到岸1022·税113」，综合研判注明「含13%增值税」 |

> 剩余待办：
> - `fetch_weekly.py` 的 `REAL_SOURCES` 继续接入资源网（价格/澳煤）、Clarkson（海运费）、华通人（各产量增速）真实源（无公开 API，需评估数据采购或抓取）。
> - 布伦特 `fetch_brent()` 用的 `ak.energy_oil_hist()` 列名需在 CI（akshare 可用环境）实测校准。
> - `IMPORT_MODEL.fx` 汇率仍写死 7.2，待接实时汇率或加录入项；`COAL_OIL_REF`、比价「历史均值」列待用真实历史序列校准（问题一剩余部分）。
> - 库存/供需阈值（`INV_REF`/`ENV_THRESH`）为经验值，待攒够历史序列后用滚动均值替代。
